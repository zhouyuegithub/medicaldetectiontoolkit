...
def get_train_generators(cf, logger):
    ...
    if cf.new_data == False:
        with open(os.path.join(cf.pp_data_path, cf.input_id_name), 'rb') as handle:
...
def get_test_generator(cf, logger):
    ...
    if cf.new_data == False:
        pp_name = None
        with open(os.path.join(cf.pp_data_path, 'fold_ids.pickle'), 'rb') as handle:
            fold_list = pickle.load(handle)
        if cf.test_for_next == False:
    ...
    if cf.new_data == True:
        ...
        if cf.test_for_next == False:
            ...
            _, test_pids, _ = read_pid(cf,cf.fold)
