...
def get_train_generators(cf, logger):
    ...
    print('len(all_pids_list): ', all_pids_list)

    if not cf.created_fold_id_pickle:#default: False
        fg = dutils.fold_generator(seed=cf.seed, n_splits=cf.n_cv_splits, len_data=len(all_pids_list)).get_fold_names()#return idx for 5 folds(0,5,170)
...
def get_test_generator(cf, logger):
    ...
    """
    print('in get_test_generator')
    print('hold_out_test_set',cf.hold_out_test_set)
...
class PatientBatchIterator(SlimDataLoaderBase):
    ...
    def __init__(self, data, cf): #threads in augmenter
        ...
        print('test data',self.dataset_pids)
