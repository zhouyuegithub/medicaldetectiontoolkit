...
class RPN(nn.Module):
    ...
    def __init__(self, cf, conv):
        ...
        # anchors per pixel == anchor_ratios
...
class Classifier(nn.Module):
    ...
    def __init__(self, cf, conv):
        ...
        if 'fpn' in cf.backbone_path:
            self.conv1 = conv(self.temp_end_filter, self.temp_end_filter * 4, ks=self.pool_size, stride=1, norm=norm, relu=cf.relu)
            self.conv2 = conv(self.temp_end_filter * 4, self.temp_end_filter * 4, ks=1, stride=1, norm=norm, relu=cf.relu)
            self.temp_end_filter = self.temp_end_filter * 4
        if 'vnet' in cf.backbone_path:
            if self.cf.multi_scale_det == False:
    ...
    def forward(self, x, rois):
        ...
        """
        x = pyramid_roi_align(self.cf,x, rois, self.pool_size, self.pyramid_levels, self.dim)
...
class Change_Vnet_Channel(nn.Module):
    """
    for multi-scale Vnet feature map channel change from [32,64,128,256,256] to [36,36,36,36,36]
    """
    def __init__(self, cf, conv):
        super(Change_Vnet_Channel, self).__init__()
        self.cf = cf
        self.out_ch = 36
        self.conv_level0 = conv(32,self.out_ch,ks=3,stride=1,pad=1,norm=cf.norm,relu=cf.relu)
        self.conv_level1 = conv(64,self.out_ch,ks=3,stride=1,pad=1,norm=cf.norm,relu=cf.relu)
...
class UpSampling(nn.Module):
    '''
    up sampling for deeply supervised
    '''
    def __init__(self, cf, conv):
        self.cf = cf
        self.upsample = nn.Upsample(scale_factor = 2,mode = 'triliner',align_corners=True)
        self.conv = conv()
    def forward(self,x):
...
class Fusion(nn.Module):
    ...
    def __init__(self, cf, conv):
        ...
        self.cf = cf
        if 'add' in self.cf.fusion_method:
            in_ch = 2
...
class Mask(nn.Module):
    ...
    def __init__(self, cf, conv):
        ...
        mask_channel = cf.end_filts[cf.pyramid_levels[0]]# this is a bug to be fixed for vnet only single scale feature can be used
...
def compute_rpn_class_loss(rpn_match, rpn_class_logits, shem_poolsize):
    ...
    :param shem_poolsize: int. factor of top-k candidates to draw from per negative sample 10
...
def compute_mrcnn_mask_loss(target_masks, pred_masks, target_class_ids):
    ...
    :return: loss: torch 1D tensor.
    """
    #print('in compute_mrcnn_mask_loss')
...
def compute_vnet_seg_loss(pred,gt):
    ...
    cardinality = torch.sum(pred + gt,dims)
...
def proposal_layer(rpn_pred_probs, rpn_pred_deltas, proposal_count, anchors, cf):
    ...
    #print('pre_nms_limit',cf.pre_nms_limit)#6000
    ...
    for ix in range(batch_scores.shape[0]):#batch size
        ...
        if batch_deltas.shape[-1] == 4:#dim = 2
            ...
            keep = nms_3D(torch.cat((boxes, scores.unsqueeze(1)), 1), cf.rpn_nms_threshold)#0.7
            norm = torch.from_numpy(cf.scale).float().cuda()
...
def pyramid_roi_align(cf,feature_maps, rois, pool_size, pyramid_levels, dim):
    """
    ...
    if 'vnet' in cf.backbone_path:
        roi_level = (5 + mutils.log2(torch.sqrt(h*w))).round().int().clamp(pyramid_levels[0], pyramid_levels[-1])#this roi from which feature level
    if 'fpn' in cf.backbone_path:
    ...
    for level_ix, level in enumerate(pyramid_levels):
        ...
        ix = roi_level == level
        #print('ix',len(ix))
        if not ix.any():#all flase reture false means roi_level not in level
            continue
...
def detection_target_layer(batch_proposals, batch_mrcnn_class_scores, batch_gt_class_ids, batch_gt_boxes, batch_gt_masks, cf):
    ...
    for b in range(len(batch_gt_class_ids)):
        ...
        if 0 not in torch.nonzero(positive_roi_bool).size():
            ...
            positive_samples = int(cf.train_rois_per_image * cf.roi_positive_ratio)#6*0.5
...
class net(nn.Module):
    ...
    def build(self):
        ...
        if self.cf.multi_scale_det == True:
            self.ChangeVnetChannel = Change_Vnet_Channel(self.cf,conv)
    ...
    def train_forward(self, batch, is_validation=False):
        ...
        if self.cf.mask_loss_flag == 'BCE':
            if self.cf.fusion_feature_method == 'after':
                mrcnn_mask_loss = compute_mrcnn_mask_loss(target_mask, mrcnn_pred_mask, target_class_ids)
            else:
                mrcnn_mask_loss = compute_mrcnn_mask_loss(target_mask, F.softmax(mrcnn_pred_mask, dim=1), target_class_ids)
        ...
        if self.cf.mask_loss_flag == 'roiDice':
            target_mask_np = target_mask.cpu().detach().numpy()[:,np.newaxis,:,:,:]
            target_masks_onehot = torch.FloatTensor(mutils.get_one_hot_encoding(target_mask_np,self.cf.num_seg_classes+1)).cuda()
            if self.cf.fusion_feature_method == 'after':
                mrcnn_mask_loss = compute_vnet_seg_loss(mrcnn_pred_mask,target_masks_onehot)#roi dice
            if self.cf.fusion_feature_method == 'before':
                mrcnn_mask_loss = compute_vnet_seg_loss(F.softmax(mrcnn_pred_mask, dim=1),target_masks_onehot)#roi dice
        if self.cf.mask_loss_flag == 'mapDice':
            if self.cf.fusion_feature_method == 'after':
                mrcnn_mask_loss = compute_vnet_seg_loss(mask_map,seg_label_onehot)
            if self.cf.fusion_feature_method == 'before':
                mrcnn_mask_loss = compute_vnet_seg_loss(F.softmax(mrcnn_pred_mask, dim=1),target_masks_onehot)#roi dice
